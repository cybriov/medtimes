...
class Employee extends Model
{
        ...
        if(!empty($default_role_id)) 
        {
             ...
             if(!empty($role_id))
             {
                 ...
                 if(!empty($role_array)) 
                 {
                     ...
                     {
                        $role_value .= '<span class="badge  badge-xs badge-success mr-1" style="background-color:'.$role_array_val['color'].'"> '.$role_array_val['name'].' </span>';
                     }
                 }                 
             }
        }
                ...
                if(count($employees) == 0) 
                {
                    ...
                    if(!empty($employees)) 
                    {
                        ...
                        if($leave_type == 1) 
                        {
                            ...
                            $table_date[] = '<div class="badge badge-soft-info" title="'.$tooltip.'" '.$ajax_pop.'>Y</div>';
                        }
                        if($leave_type == 2) 
                        {
                            $tooltip = __('Other Leave').'  '.$start_date.' &nbsp; - &nbsp; '.$end_date;
                            $table_date[] = '<div class="badge badge-soft-success" title="'.$tooltip.'" '.$ajax_pop.' >Y</div>';
                        }
                    }
                }
            }
    ...
    public function getWorkSchedule($id = 0, $week = 0, $location_id = 0)
    {
        ...
        if($id != 0)
        {
            ...
            foreach($week_date as $key => $date)
            {
                ...
                $roster_calls = Rotas::select('*',DB::raw('TIMEDIFF(end_time,start_time) as time_between'))->Where('location_id',$location_id)->Where('user_id',$id)->Where('is_call','1')->Where('rotas_date',$date)->get()->toArray();
                if(!empty($rotas)) 
                {
                    $rotas_time1 = '';
                    foreach($rotas as $rota)
                    {
                        ...
                        if ($rota['is_call'] == 1) {
                            $rotas_time1 .= '<div class=" rounded rotas_time rotas_time1 draggable-class '.$publish_shift.'" data-rotas-id="'.$rota['id'].'" style=" '.$ccc.'">
                                            <b class="text-dark">'.$timee1.'</b><sup class="badge1" style="background-color: #8492a6;">OT</sup><br>
                                            <span class="text-dark"> '.$role_name.' </span>
            ...
            if(!empty($employee))
            {
                foreach($week_date as $key => $date)
                {
                    ...
                    if($login_employee->acount_type == 1 || in_array($location_id,$manage_location1))
                    {
                        ...
                        {
                            $add = '<a href="#" class="action-item mr-2 add_rotas "  data-size="md" data-ajax-popup="true" data-title="'. __('Add New Shift').'" title="'. __('Add New Shift').'" data-url="'. route('rotas.create',['id'=>$id,'date'=>$date]).'"><i class="fas fa-plus" ></i></a>';
                        }
                    }
                ...
                foreach($work_schedule as $days) 
                {
                    ...
                    {                        
                        ...
                        if($login_employee->acount_type == 1 || in_array($location_id,$manage_location1))
                        {
                            if($manage_add_shift != 0)
                            {
                                $add = '<a href="#" class="action-item mr-2 add_rotas"  data-size="md" data-ajax-popup="true" data-title="'. __('Add New Shift').'" title="'. __('Add New Shift').'" data-url="'. route('rotas.create',['id'=>$id,'date'=>$week_date[$i]]).'" ><i class="fas fa-plus"></i></a>';
                            }
                        }
                ...
                if(!empty(($profile_data_df['custom_day_off'])))
                {
                    ...
                    if(in_array($date,$custom_day_off))
                    {
                        ...
                        if($login_employee->acount_type == 1 || in_array($location_id,$manage_location1))
                        {
                            if($manage_add_shift != 0)
                            {
                                $add = '<a href="#" class="action-item mr-2 add_rotas "  data-size="md" data-ajax-popup="true" data-title="'. __('Add New Shift').'" title="'. __('Add New Shift').'" data-url="'. route('rotas.create',['id'=>$id,'date'=>$week_date[$ci]]).'" ><i class="fas fa-plus"></i></a>';
                            }
                        }
        ...
        $time_counter = ($admin_idss == 1) ? '<span>'.$working_hour.' / '.$shift_role.'</span>' : '';
        return '<tr class="d-nowne" data-user-id="'.$id.'"><td class="text-center">'.$user_profile_img.'<div><span>'.$employee['first_name'].' '.$employee['last_name'].'</span>
        <br>'.$time_counter.' </div></td>'.$table_date[0].' '.$table_date[1].' '.$table_date[2].' '.$table_date[3].' '.$table_date[4].' '.$table_date[5].' '.$table_date[6].'</tr>';
    }
    ...
    public static function getCompanyWeeklyUserSalary($week = 0, $create_by = '', $first_location = 0, $role_id = 0)
    {
        ...
        if($emp_show_rotas_price == 1)
        {
            ...
            //$tr2 = '<!tr class="text-center"> <th><span>'.__('Cost').'</span> <span>'.User::priceFormat($weekly_money).'</span></th> <td class="min_width-170">'.User::priceFormat($tr_cost[0]).'</td> <td class="min_width-170">'.User::priceFormat($tr_cost[1]).'</td> <td class="min_width-170">'.User::priceFormat($tr_cost[2]).'</td> <td class="min_width-170">'.User::priceFormat($tr_cost[3]).'</td> <td class="min_width-170">'.User::priceFormat($tr_cost[4]).'</td> <td class="min_width-170">'.User::priceFormat($tr_cost[5]).'</td> <td class="min_width-170">'.User::priceFormat($tr_cost[6]).'</td> </tr>';
        }
