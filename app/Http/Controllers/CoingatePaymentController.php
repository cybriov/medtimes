    ...
    public function coingatePlanGetPayment(Request $request)
    {
        ...
        if($plan)
        {
            try
            {
                ...
                {
                    return redirect()->rout('plan.index')->with('success', __('Plan activated Successfully.'));
                }
                else
                {
                    return redirect()->rout('plan.index')->with('error', $assignPlan['error']);
                }
            }
            catch(\Exception $e)
            {
                return redirect()->rout('plan.index')->with('error', __('Transaction has been failed.'));
            }
        }
        else
        {
            return redirect()->rout('plan.index')->with('error', __('Plan is deleted.'));
        }
    }
