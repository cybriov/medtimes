            ...
            {           
                    ...
                    if(!empty($rotas))
                    {
                        ...
                        foreach($rotas as $rota)
                        {
                            ...
                            $count_role_id[$date][$rota['role_id']][$rota['location_id']][] = array(
                                ...
                                'location_name'     => $rota['location_id'],
                                //'location_name'     => $location->name,
                                'start_time'        => $rota['start_time'],
                                'end_time'          => $rota['end_time'],
                                'data'              => $employee_data[$rota['user_id']].' ('.$rota['start_time'].' - '.$rota['end_time'].') '.$rota['location_id'],
                            );
                        }
                    }
                }
                ...
                foreach($count_role_id as $feed_key => $count_role_ids)
                {
                    ...
                    if(!empty($count_role_ids))
                    {
                        ...
                        $priceee  = ($emp_show_rotas_price == 1) ? ' ' : '';
                        ...
                        $feed_calender[$i] = array(
                            ...
                            'html' => '<div>'.$roll_cnt.'<div class=" '.$text_color.' opacity-50 mt-2" style=" font-size: 12px; "> <span title="'.$cnt_employee1.' '.__('Employees').'"><i class="fas fa-user" aria-hidden="true"></i> <span>'.$cnt_employee1.'</span></span> &nbsp;&nbsp;  '.$priceee.'  </div></div>',
                        );
                    }
                ...
                return view('home',compact('feed_calender','roles','locations'));
            }
        } else {
        ...
        foreach($count_role_id as $feed_key => $count_role_ids)
        {
            ...
            if(!empty($count_role_ids))
            {
                ...
                $feed_calender[$i] = array(
                    ...
                    'end' => $feed_key,
                    'className' => 'bg-transparent',
                    'html' => '<div>'.$roll_cnt.'<div class="'.$text_color.' opacity-50 mt-2" style=" font-size: 13px; "> <span data-toggle="tooltip" title="'.$cnt_employee1.' '.__('Employees').'"><i class="fas fa-user" aria-hidden="true"></i> <span>'.$cnt_employee1.'</span></span> &nbsp;&nbsp;  '.$priceee.'  </div></div>',
                );
            }
