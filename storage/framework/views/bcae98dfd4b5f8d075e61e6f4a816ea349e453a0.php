...
<head>
    ...
    <meta name="description" content="A platform that relieves medical professionals from the pain of performing manual HR functions like rostering and managing timesheets.">
    ...
    <title><?php echo e(__('Medtimes')); ?> - A Rostering Tool For Medical Professionals</title>
                ...
                success: function(data) {
                    if (data == "error") {
                        ...
                        $(".tooltip1").hover(function(e) {
                            var x = $(this).position();
                            $(this).find(`#ul-${section_type}`).find('.tooltip1text').css({
                                'top': x.top
                            });
                            $(this).find(`#ul-${section_type}`).find('.tooltip1text').css(
                                "visibility", "visible");
                            $(this).find(`#ul-${section_type}`).find('.tooltip1text').css("opacity",
                                1);
                        });
                        ...
                        if (object.section_type != "section-plan") {
                            ...
                            key.forEach(function(val) {
                                if (val == "custom_class_name") {
                                    $(`#setting-modal-section-${section_id}`).find(`#${val}`).val(
                                        content[val]);
                                    $(`#${section_type}`).addClass(content[val]);
                                }
                                if (val == "logo") {
                                    $(`#${section_type}`).find(`#${val}`).attr('src',
                                        `<?php echo e($image_path . '/${content[val]}'); ?>`);
                                    $(`#setting-modal-section-${section_id}`).find(`#${val}`).find(
                                        'img').attr('src',
                                        `<?php echo e($image_path . '/${content[val]}'); ?>`);
                                }
                                if (val == "image") {
                                ...
                                var img_change_btn = $(`#setting-modal-section-${section_id}`).find(
                                    `#${val}`).find('a');
                                var file_input = $(`#setting-modal-section-${section_id}`).find(
                                    `#${val}`).find('input');
                                ...
                                file_input.change(function() {
                                    ...
                                    var ext = url.substring(url.lastIndexOf('.') + 1)
                                        .toLowerCase();
                                    if (this.files && this.files[0] && (ext == "gif" ||
                                            ext == "png" || ext == "jpeg" || ext == "jpg"
                                        )) {
                                        var reader = new FileReader();

                                        reader.onload = function(e) {
                                            $(`#setting-modal-section-${section_id}`)
                                                .find(`#${val}`).find('img').attr('src',
                                                    e.target.result);
                                            $(`#${section_type}`).find(`#${val}`).attr(
                                                'src', e.target.result);
                                        }

                                        reader.readAsDataURL(this.files[0]);
                                    } else {
                                        $(`#setting-modal-section-${section_id}`).find(
                                ...
                                if (val == "button") {
                                    ...
                                    if (section_type == "section-1") {
                                        ...
                                        $(`#${section_type}`).find(`#${val}`).attr('href', content[
                                            val].href);
                                    }

                                    $(`#setting-modal-section-${section_id}`).find(`#${val}`).find(
                                        `input[name="button[text]"]`).val(content[val].text);
                                    $(`#setting-modal-section-${section_id}`).find(`#${val}`).find(
                                        `input[name="button[href]"]`).val(content[val].href);
                                    $(`#setting-modal-section-${section_id}`).find(`#${val}`).find(
                                        `input[name="button[text]"]`).keyup(function() {
                                        var text_value = $(
                                                `#setting-modal-section-${section_id}`)
                                            .find(`#${val}`).find(
                                                `input[name="button[text]"]`).val();
                                        $(`#${section_type}`).find(`#${val}`).html(
                                            text_value);
                                    });
                                    $(`#setting-modal-section-${section_id}`).find(`#${val}`).find(
                                        'input[name="button_href"]').change(function() {
                                        var href_value = $(
                                                `#setting-modal-section-${section_id}`)
                                            .find(`#${val}`).find(
                                                `input[name="button[href]"]`).val();
                                        $(`#${section_type}`).find(`#${val}`).attr('href',
                                            href_value);
                                    });
                                }
                                if (val == "menu") {
                                    ...
                                    $(`#setting-modal-section-${section_id}`).find(`#${val}`).html(
                                        input_menu_html);
                                    menu.forEach(function(val) {
                                        $(`#setting-modal-section-${section_id}`).find(
                                            `#${val.menu}`).keyup(function() {
                                ...
                                if (val == "text") {
                                    ...
                                    menu_key.forEach(function(element) {
                                        ...
                                        $(`#${section_type}`).find(`#${element}`).html(
                                            content[val][element]);
                                        $(`#setting-modal-section-${section_id}`).find(
                                            `#${element}`).val(content[val][element]);
                                        $(`#setting-modal-section-${section_id}`).find(
                                ...
                                if (val == "system") {
                                    ...
                                    var no = 1;
                                    var collpse_html = ``;
                                    $(`#setting-modal-section-${section_id}`).find('#add_class_btn')
                                        .click(function() {
                                            system_add_conetent('testnomial_add_class_form',
                                                section_id)
                                        });
                                    system.forEach(function(element) {
                                        var tab_content_div = '';
                                        var li_id = element.name + '_' + element.id + '_' +
                                            section_id;
                                        var active_class = "";
                                        ...
                                        tab_html +=
                                            `<li><a data-toggle="tab" href="#${li_id}" class="${active_class}">${element.name}</a></li>`;
                                    ...
                                    $(`#setting-modal-section-${section_id}`).find("#accordion")
                                        .html(new_tab_form);
                                    $(`#setting-modal-section-${section_id}`).find("#accordion")
                                        .append(collpse_html);
                                }
                                if (val == "testimonials") {
                                    ...
                                    $(`#setting-modal-section-${section_id}`).find('#add_class_btn')
                                        .click(function() {
                                            system_add_conetent('form_testinomials_add_class',
                                                section_id)
                                        });
                                    $(`#setting-modal-section-${section_id}`).find('#add_new_test')
                                        .click(function() {
                                            system_add_conetent('form_testinomials_add_new',
                                                section_id)
                                        });
                                    ...
                                    $(`#${section_type}`).find('#customers-testimonials').html(
                                        testimonials_html);

                                    var carousel = $(`#${section_type}`).find(
                                    ...
                                    function checkClasses(arg) {
                                        ...
                                        if (arg == "" || arg == null) {
                                            FindActive = jQuery('.owl-stage').find(
                                                '.owl-item.active');
                                        }
                                        ...
                                        for (var i = 0; i < 2; i++) {
                                            ...
                                            if (i == 0) {
                                                ...
                                                $(FindActive[i]).find('.item').css('opacity',
                                                    '0.5');
                                            }
                                        }
                                    }
                                    ...
                                    $(`#setting-modal-section-${section_id}`).find(
                                        '#accordion_testimonials_dynamic_section').html(
                                        testinomials_collpse_html);
                                }
                                ...
                                if (val == "footer") {
                                    ...
                                    footer.forEach(function(element) {
                                        if (element == "logo") {
                                            ...
                                            $(`#${section_type}`).find("#logo").find('img')
                                                .attr('src',
                                                    `<?php echo e($image_path . '/${logo_img}'); ?>`
                                                    );
                                            ...
                                            var menu_html = "";
                                            var menu_modal_html = "";
                                            content[val][element].forEach(function(
                                                menu_list) {
                                                ...
                                                menu_html +=
                                                    `<div class="col-lg-3 top-part"><h3 id="footer_menu_${footer_menu_id}">${menu}</h3><ul>`;
                                                var no = 1;
                                                data.forEach(function(data_val) {
                                                    menu_inner_html +=
                                                        `<li><a href="${data_val.menu_href}" id="footer_menu_${footer_menu_id}_${no}">${data_val.menu_name}</a></li>`;
                                            ...
                                            $(`#${section_type}`).find("#footer_section")
                                                .append(menu_html);
                                            $(`#setting-modal-section-${section_id}`).find(
                                                "#footer_modal_footer_menu").html(
                                                menu_modal_html);
                                            ...
                                            content[val][element].forEach(function(
                                                ...
                                                var menu_inner_html = "";
                                                menu_html +=
                                                    `<div class="col-lg-3 top-part"><h3 id="contact_app_menu">${menu}</h3><ul>`;
                                                menu_modal_html += `<div class="card mb-2">
                                                                            <div class="card-header"><h6>${menu}</h6></div>
                                                                            <div class="card-body">
                                                                                <div class="row">
                                                ...
                                                data.forEach(function(data_val) {
                                                    menu_inner_html +=
                                                        `<li><a href="${data_val.image_href}"><img src="<?php echo e($image_path . '/${data_val.image}'); ?>" id="section_footer_contact_${section_id}_${no}" alt="logo"></a></li>`;
                                                    menu_modal_html += `<div class="form-group">
                                            ...
                                            $(`#${section_type}`).find("#footer_section")
                                                .append(menu_html);
                                            $(`#setting-modal-section-${section_id}`).find(
                                                "#contact_menu").html(menu_modal_html);
                                        } else if (element == "bottom_menu") {

                                            var text = 'Medtimes © ' + new Date().getFullYear() +
                                                '. ' + content[val][element].text;
                                            var text1 = content[val][element].text;
                                            var data = content[val][element].data;
                                            $(`#${section_type}`).find("#footer_bottom")
                                                .find('#text').html(text);
                                            var menu_html = "";
                                            ...
                                            data.forEach(function(data_val) {
                                                menu_html +=
                                                    `<li><a href="${data_val.menu_href}" id="bottom_menu_${no}">${data_val.menu_name}</a></li>`;
                                                menu_modal_html += `<div class="form-group">
                                            ...
                                            $(`#${section_type}`).find("#footer_bottom")
                                                .find('#data').html(menu_html);
                                            $(`#setting-modal-section-${section_id}`).find(
                                                "#footer_modal_footer_bottom").html(
                                                menu_modal_html);

                                            $(`#setting-modal-section-${section_id}`).find(
                                                `#footer_modal_footer_bottom`).find(
                                                `#text`).keyup(function() {
                                                var text_value = 'Medtimes ©' + new Date()
                                                    .getFullYear() + ' ' + $(this)
                                                    .val();
                                                $(`#${section_type}`).find(
                                                    "#footer_bottom").find(
...
<?php /**PATH /home/jovickxe/medtimes.co/resources/views/layouts/landing.blade.php ENDPATH**/ ?>